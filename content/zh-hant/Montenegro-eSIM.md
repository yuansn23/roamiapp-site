...
type: esim
country_meta:
  core_keyword: 蒙特內哥羅 eSIM
  scenario_keywords: 城市漫步, 海灘渡假, 購物, 美食探索, 古城觀光
  operators: Crnogorski Telekom, Telenor (OneCG), M:tel
  competitors: Airalo, Holafly, Nomad, Ubigi, Yesim
  main_cities: 波德里查, 布德瓦, 科托爾, 蒂瓦特, 赫爾採格諾維
...
activation_steps:
  ...
  - num: '2'
    ...
    - 蒙特內哥羅 的 eSIM 設定檔會自動下載 – 點擊 **『下一步』**
    - 為其命名（例如「Roami 蒙特內哥羅」）並將其設為 **行動數據的預設選項**
...
faq_section:
  ...
  - id: faq-airport
    icon: plane
    tab_title: 機場
    title: 蒙特內哥羅 機場：如何使用 蒙特內哥羅 eSIM
    questions:
    ...
    - q: 在蒙特內哥羅機場轉機時，eSIM會因為短暫停留而產生額外費用嗎？
      a: 我們的eSIM方案以天數計費，例如3天、7天等，從啟用開始計算，而非依賴在機場的實際停留時間。如果您在轉機期間僅短暫連線（如1-2小時），不會產生額外費用，因為您已支付整個方案費用。注意，若您在轉機時誤連至克羅埃西亞基地台（如杜布羅夫尼克機場附近），可能產生跨境費用，但我們的eSIM已鎖定蒙特內哥羅網路，避免此情況。
    - q: 蒂瓦特機場有貨幣兌換處嗎？建議用現金還是信用卡支付交通費？
      a: 蒂瓦特機場內有Addiko Bank的兌換櫃檯（位於行李轉盤旁），但匯率較差（約5%手續費）。建議在機場ATM直接提領歐元，或事先準備歐元現金。計程車和接駁車普遍接受現金，少數也接受信用卡，但可能有最低金額限制。若要使用叫車App（如Moja
        Taxi），通常需綁定信用卡。機場距離市區僅5公里，也可步行至附近巴士站（約10分鐘）搭乘客運，費用1.5歐元。
  - id: faq-transport
    icon: car
    tab_title: 交通
    ...
    - q: 在蒙特內哥羅使用共享單車或電動滑板車，是否會用到大量手機數據？哪裡有租借點？
      a: 布德瓦和科托爾的共享電動滑板車（如CarGo）透過App租借，需使用數據定位和支付，每次租借約消耗5-10MB。租借點集中在海濱步道和古城入口，費用約每分鐘0.2歐元。我們的eSIM方案數據充足，不用擔心用量。建議提前下載App並註冊（需信用卡）。注意部分區域（如石階路段）禁止騎行，請遵守當地規定。
    - q: 從蒙特內哥羅的赫爾採格諾維到科托爾，搭船還是巴士更方便？網路在船上穩定嗎？
      a: 赫爾採格諾維到科托爾的船班（約1小時），由Kotor Lines營運，夏季每日5班，費用12歐元。船上靠近科托爾灣時，Telenor網路訊號良好，但行經海峽中央可能短暫中斷。巴士（約1.5小時）費用6歐元，經由海岸公路，全程網路穩定。船班提供更佳風景，但建議攜帶暈船藥。兩者均可在當地旅遊資訊中心購票。
  - id: faq-hotels
    icon: hotel
    tab_title: 住宿
    title: 在 蒙特內哥羅 的飯店使用 蒙特內哥羅 eSIM 連線
    questions:
    - q: 在布德瓦的五星級飯店Splendid Conference & Spa Resort，房內Wi-Fi速度快嗎？是否建議使用eSIM？
    ...
    - q: 蒂瓦特機場附近的飯店（如Hotel Pine），如何確保網路順暢？
      a: Hotel Pine提供免費Wi-Fi，速度約20Mbps，但近機場可能受干擾。使用我們的eSIM連接Telenor網路，在飯店區域下載速度可達50Mbps。若您在晚上抵達，需即時使用網路規劃行程，建議先在機場啟用eSIM。飯店附近有咖啡廳（如Caffe
        Bar Portaz）提供強Wi-Fi，可作為備用。
    - q: 在蒙特內哥羅的沿海度假村（如布德瓦），eSIM的數據方案是否足夠一家四口使用？
      a: 一家四口若共用一個eSIM熱點，每日使用社交媒體、導航和串流影片約消耗3-5GB。我們的7天15GB方案（約15歐元）足夠，但注意同時串流高畫質影片可能加速用量。建議每位家人使用自己的設備連線，或至少配備2個eSIM分擔流量。Telenor網路在度假村區域支援多設備連接，延遲穩定。若超過每日5GB，可能降速至2Mbps，仍可收發郵件。
  - id: faq-attractions
    icon: landmark
    tab_title: 景點
    ...
    - q: 從科托爾出發一日遊，參觀洛夫琴國家公園和Njegoš陵墓，沿途網路覆蓋是否穩定？
  ...
  - id: faq-emergency
    icon: alert-triangle
    tab_title: 緊急情況
    title: 在 蒙特內哥羅 使用 蒙特內哥羅 eSIM 進行緊急聯絡
    questions:
