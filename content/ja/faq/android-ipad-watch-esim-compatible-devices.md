...
監修: 本記事は各メーカー公式の技術仕様書、[**GSMA eSIM仕様書（SGP.22）**](https://www.gsma.com/solutions-and-impact/technologies/esim/)、[**Google Pixel eSIMサポートページ**](https://support.google.com/pixelphone/answer/14853135?hl=ja)、[**Samsung公式サポート**](https://www.samsung.com/jp/support/)、[**Apple公式サポート**](https://support.apple.com/ja-jp/118569)をはじめとする各社公式情報、および各キャリアの公開情報に基づいています。2026年6月時点の情報です。


Androidスマホ・iPad・Apple Watch——**eSIM対応機種**をメーカー別に全機種網羅します。Google Pixel、Samsung Galaxy、Xperia、AQUOS、Xiaomi、OPPO、Motorolaまでカバー。Android側の対応端末を探している方、**iPad eSIM**や**Apple Watch eSIM**を検討している方に、この1記事で全てがわかる完全ガイドです。
...
例えば**Google Pixelは2020年のPixel 5以降、eSIMを2つ同時に保持・運用できる「デュアルeSIM」に対応**しています。この機能の基盤となっているのは、[**GSMA**](https://www.gsma.com/solutions-and-impact/technologies/esim/)が策定したMEP（Multiple Enabled Profiles）という規格で、複数のプロファイルの同時アクティブ化を可能にするものです。PixelはiPhoneがiOS 17.4でデュアルeSIMに正式対応したタイミングより、約4年も先行していました。また、**Android 13以降**ではOSレベルでeSIM管理機能が強化され、キャリアアプリを介さずにプロファイルの切り替え・削除が可能になりました。さらに、**Android 15ではMEPのネイティブサポート**が強化され、対応端末では2つのeSIMを同時アクティブにできます。
...
つまり「iPhoneは全機種対応で安心だが、対応端末を選べばiPhone以上に柔軟な運用ができる」というのが実態です。**重要なのは「対応・非対応」だけでなく「どの程度の自由度があるか」まで理解すること**です。
...
**① デュアルeSIMの実績が最も長い**
Pixel 5（2020年）からデュアルeSIMに対応しており、業界で最も長い運用実績があります。OSのアップデートに合わせてeSIM管理機能も継続的に改善されており、プロファイルの切り替え速度・安定性は、対応Android端末の中で最も信頼できます。
...
公式情報は[**Google PixelのeSIMサポートページ**](https://support.google.com/pixelphone/answer/14853135?hl=ja)で確認できます。
...
「esim pixel」「google pixel esim」いずれのキーワードで調べた場合も、Pixel 6以降のモデルを第一候補に挙げたい。デュアルeSIMの安定性が格段に向上しており、仕事用・プライベート用の回線を1台にまとめたいユーザーに最適です。
...
対策：対応GalaxyでeSIMを最大限活用したいなら、可能な限りSIMフリーのグローバル版を選ぶのが最も確実です。中古市場でもグローバル版Galaxyは入手可能で、eSIMの自由度はキャリア版と比べて格段に高いです。
...
eSIMの利用可否や対応回線は、Samsung公式の[**eSIMと対応通信事業者の案内ページ**](https://www.samsung.com/jp/support/mobile-devices/galaxy-esim-and-supported-network-carriers/)で確認できます。
...
eSIMの登録手順は、ソニーが公開するXperia 10 VII（XQ-FE44）の[**公式ヘルプガイド**](https://helpguide.sony.net/mobile/xperia-10m7/v1/ja/contents/configure_esim.html)に、設定アプリからの操作ステップとして記載されています。
...
なおXiaomiは現時点でeSIMに関する専用サポートページを公開しておらず、対応状況は機種ごとの技術仕様や正規代理店の案内を個別に確認する必要があります。
...
OPPOもeSIM専用のサポートページは設けておらず、機種の対応状況は[**OPPO公式サポート（日本）**](https://support.oppo.com/jp/)から問い合わせ・確認する流れになります。
...
理由は日本のキャリアが端末のファームウェアをカスタマイズしているからです。ドコモ・au・ソフトバンクは、販売する端末のeSIM管理モジュールに**自社の制御を入れています**。具体的には：
...
物理SIM + eSIMのデュアルSIMデュアルスタンバイ（DSDS）は、ほぼすべての対応Android端末でサポートされています。ただし、メーカーによって動作の安定性や設定の柔軟性に差があります：
...
DSDSを活用すれば、メイン回線（物理SIM）は音声通話専用、サブ回線（eSIM）はデータ通信専用——といった使い分けが可能になり、通信費の最適化やエリア補完に役立ちます。サブ回線の選択肢は[**日本向けeSIMプラン**](/japan-esim/)の比較ページも参考になります。

### 実践的な対策

日本でAndroid端末のeSIMを最大限活用したいなら、以下の順序で検討してください。
...
iPadのSIMトレイは開閉頻度は低いものの、接触不良はiPadのよくある故障原因のひとつです。eSIMなら物理的な接触部分がないため、このリスクがゼロになります。

**② 防水性能の維持——SIMトレイを開ける回数が減る**
iPadを水辺やキッチンで使うユーザーにとって、SIMトレイのパッキン劣化は気になるポイント。eSIMならトレイを開ける必要がないため、経年劣化による防水性能の低下を抑えられます。

**③ 海外渡航時の圧倒的な柔軟性**
現地の物理SIMと日本のeSIM、または日本の物理SIMと現地のeSIM——2重の選択肢が持てます。特にiPadを**サブ端末として海外に持ち出す場合**、eSIMだけで完結する運用が非常に便利です。渡航先が北米なら[**アメリカのeSIM**](/united-states-esim/)など現地プランを直接ダウンロードする形も選べます。
...
申込の前提はどのキャリアも共通で、ペアリング済みのiPhone回線に追加契約する形になります。単体での新規契約はできず、対象プランも各社で定められているため、手続き前に必ず公式ページで条件を確認しましょう。

- [ドコモのeSIM案内（申込・再発行）](https://www.docomo.ne.jp/support/uimcard/esim/)
- [auのeSIM開通手続き](https://www.au.com/support/service/mobile/procedure/sim/esim/)
- [ソフトバンクのeSIM製品ページ（対応機種・プラン）](https://www.softbank.jp/mobile/products/sim/esim/)
...
**④ バッテリー消費が増加する**
セルラー版Apple WatchはeSIM通信中にバッテリーを消費します。特にApple Watch SEやSeries 4〜6など古いモデルでは、GPS + Cellularモデルで常時通信している場合、バッテリー駆動時間が半減することもあります。Apple Watch Ultra / Ultra 2は大容量バッテリーを搭載しているため比較的安心ですが、それでも1日1回の充電は必須と考えてください。
...
中古のAndroidスマホでeSIMを使う場合、**新品購入とは異なる注意点**がいくつかあります。端末側の対応可否は[**互換性チェックツール**](/compatibility/)でも照会できます。
...
前所有者が料金未払いや盗難届を出している場合、その端末のIMEIがブラックリストに登録され、日本のキャリアで通信できなくなる可能性があります。購入前にIMEIを確認し、キャリアのサポートに問い合わせることを推奨します。
...
購入後は、[**無料で試せるトライアルeSIM**](/free-esim/)で通信できるかを確かめてから本契約に進むと安心です。


## Android・iPad・Apple WatchのeSIM対応Q&A
...
**A.** はい。ドコモ・au・ソフトバンクの各キャリアでiPadのeSIM契約が可能です。また、IIJmioやBIC SIM、日本通信SIMなどのMVNOでもiPad向けデータeSIMを提供しています。楽天モバイルのiPad eSIMは2026年現在対応準備中との情報があります。iPadでeSIMを使う際は、音声通話は非対応の「データ専用」となる点に注意してください。最新の対応状況はAppleの公式情報でご確認いただけます。

### Q4. iPhoneに機種変更したら、Androidで使っていたeSIMはどうなりますか？
**A.** eSIMプロファイルは端末のeUICCチップに紐づいているため、AndroidからiPhoneに直接プロファイルを移行することはできません。 必ずキャリアまたはMVNOのマイページからeSIMの再発行（再ダウンロード）手続きが必要です。物理SIMのような「カードを抜き差しするだけ」の移行はできないため、機種変更の際は事前に再発行手続きを確認しておきましょう。iPhoneのeSIM対応機種については[**iPhone編の対応機種ガイド**](/faq/iphone-esim-compatible-models-list/)をご覧ください。
...
## まとめ：Android・iPad・WatchのeSIM対応、選び方の優先順位
...
特にAndroidの場合は、日本キャリア版の制約を理解せずに購入すると後悔します。この記事を参考に、あなたの使い方に最適な**eSIM対応機種**を選んでください。

確認方法や設定手順は[**設定・アクティベーションの手順書**](/faq/esim-setup-activation-device-change-manual/)にまとめています。iPhoneの対応状況は[**iPhoneの対応一覧まとめ**](/faq/iphone-esim-compatible-models-list/)で確認できます。

---
*本記事の内容は2026年6月時点の各メーカー・各キャリアの公開資料に基づいています。eSIM対応状況はファームウェアのバージョンやキャリアポリシーにより変わりうるため、最新情報は各公式サイトでご確認ください。*